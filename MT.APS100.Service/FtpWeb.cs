...
namespace MT.APS100.Service
{
    public class FtpWeb
    {
        ...
        public void Download(string sourcePath, string destPath)
        {
            ...
            try
            {
                FileStream outputStream = new FileStream(destPath, FileMode.Create);

                reqFTP = (FtpWebRequest)FtpWebRequest.Create(new Uri("ftp:/" + sourcePath));
        ...
        public bool FileExists(string path)
        {
            try
            {
                ...
                ftpURI = "ftp:/" + path.Substring(0, path.Length - fileName.Length);
                string[] fileList = GetFileList("*.*");
                foreach (string str in fileList)
                {
        ...
        //判断文件的目录是否存,不存则创建  
        public void MakeDirs(string destFilePath)
        {
            string tempFTPURI = ftpURI;
            //
